...
import { toggleIngredientDone } from '@/lib/actions/cooking'
...
export async function POST(
  ...
  try {
    ...
    await toggleIngredientDone(parseResult.data.id, parseResult.data.done)
    return NextResponse.json({ success: true })
  } catch (error: any) {
    const msg = error?.message || '保存失败'
    const isUserError = ['无权操作', '食材不存在', '请先登录'].some(k => msg.includes(k))
    return NextResponse.json(
